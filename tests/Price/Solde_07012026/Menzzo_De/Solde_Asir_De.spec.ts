...
const csvfile = './data/Solde_Asir_Hiver2026.csv';
...
test.describe(`Solde Asir De (All products from CSV)`, () => {
    ...
    for (const product of Price_Update) {
        ...
        test(`Test SKU: ${product.SKU}, ID: ${product.id}`, async ({ page }) => {
            ...
            const unavailableMessage = page.locator('.message.notice:has-text("nicht mehr verfügbar"), .message.notice:has-text("no longer available")');
            ...
            expect.soft(promoTagText, `Expected a sale tag for SKU ${product.SKU}`).toMatch(/SALE|ANGEBOT|PROMO/i);
