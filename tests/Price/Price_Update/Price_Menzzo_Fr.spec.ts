...
test.describe(`Check Price Update Fr`, () => {
  ...
  for (const product of Price_Update) {
    ...
    test(`FR SKU test #${product.SKU}`, async ({ page }) => {

      // Pick a product: specific ID
      let Product;
      ...
      if (product.id) {
        ...
        attachment('Product not found', `Product with ID ${Product.id} not found in CSV`, 'text/plain');
      } else {
