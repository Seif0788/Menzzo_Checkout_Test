...
test('Check_Maouro_Es_product_page', async ({ page }) => {
    // Increase timeout to 1 hour to handle the loop
    test.setTimeout(3600000);

    // --- Open menzzo.es ---
    await page.goto('https://www.menzzo.es');
    allure.attachment('Console Log', "🚪 Menzzo.es was opened", 'text/plain');
    ...
    for (const product of productsToProcess) {
        ...
        try {
            ...
            const url = `https://www.menzzo.es/catalog/product/view/id/${product.entity_id}`;
