...
test('add_to_cart_checkout', async ({ page }) => {
  // 1️⃣ Go to homepage
  await page.goto('https://www.menzzo.it/', { waitUntil: 'domcontentloaded' });
  ...
  // Select category
  await clickElementByText(page, "poltrone");
  ...
  // 4️⃣ Go to cart
  await clickElementByText(page, "Vai al tuo carrello e ordina");
  ...
  await Promise.all([
    ...
    clickElementByText(page, "Conferma il tuo carrello", 10000, { debug: true })
  ]);
